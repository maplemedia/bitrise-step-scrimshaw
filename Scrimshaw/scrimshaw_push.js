...
async function main() {
    ...
    if (IBC.hasOwnProperty('github_owner') && IBC.hasOwnProperty('github_repo')) {
        ...
        var version = await exec_cmd.execShellCommandProjDir(`fastlane run get_version_number xcodeproj:"${IBC.xcodeproj_path}"`, IBC);

        // Pushing the B tag will auto-trigger a publish workflow with this version.
        await exec_cmd.execShellCommandSourceDir(`git push --tags --set-upstream origin Scrimshaw-${version}`);
